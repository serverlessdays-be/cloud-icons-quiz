...
} from "./serviceApiIntegration";

export const getNewListOfQuestions = async (cloudProviderName) => {
  const services = await listServicesByCloudProvider(cloudProviderName);
  if (services) {
...
export function createListOfAnswers(serviceNames, correctAnswer) {
  const wrongAnswers = get3RandomElementsFromListExceptFor(
    serviceNames,
    correctAnswer
  );
